package org.apache.dragon.commons.redis;

import org.apache.log4j.Logger;
import org.junit.AfterClass;
import org.junit.BeforeClass;
import org.junit.Test;

import redis.clients.jedis.JedisPool;

/**
 * test for RedisFactory
 * 
 * @author Wenlong Meng(wenlong.meng@gmail.com)
 * @version 1.0 at Jul 31, 2013
 * @since 1.0
 */
public class RedisFactoryTest {

	// Local variables
	/**
	 * logger
	 */
	private static Logger logger = Logger.getLogger(RedisFactoryTest.class);

	// Logic
	/**
	 * test getPool
	 */
	@Test
	public void getPool() {
		logger.debug("Begin: getPool()...");
		JedisPool result = RedisFactory.getPool();
		String info = result.getResource().info();
		logger.debug("redis info: " + info);
		logger.debug("End: result = " + result);
	}
	
	/**
	 * allocate expensive external resources(like logging into a database) before any of the test methods in the class.
	 */
	@BeforeClass
	public static void init() {
		logger.debug("Begin: init()...");
		logger.debug("End: init()!");
	}

	/**
	 * releases any resource that you allocate expensive external resources in a {@link BeforeClass} method after all
	 * the tests in the this class have been run. this method guaranteed to run.
	 */
	@AfterClass
	public static void destory() {
		logger.debug("Begin: destory()...");
		logger.debug("End: destory()!");
	}

}
